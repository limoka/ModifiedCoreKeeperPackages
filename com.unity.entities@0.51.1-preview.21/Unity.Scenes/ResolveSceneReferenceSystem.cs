    ...
    partial class ResolveSceneReferenceSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            if (!m_AddScenes.IsEmptyIgnoreFilter)
            {
                ...
                using (var addScenes = m_AddScenes.ToEntityArray(Allocator.TempJob))
                {
                    ...
                    for (int i = 0; i != addScenes.Length; i++)
                    {
                        ...
                        var scene = EntityManager.GetComponentData<Entities.SceneReference>(sceneEntity);
            ...
            foreach (var change in _Changed)
            {
                ...
                // Resolve new state
                var scene = EntityManager.GetComponentData<Entities.SceneReference>(change.UserKey);
            ...
            Entities.WithStructuralChanges().WithNone<DisableSceneResolveAndLoad, ResolvedSectionEntity>().ForEach(
                (Entity sceneEntity, ref RequestSceneHeader requestHeader, ref Entities.SceneReference scene,
        ...
        protected override void OnCreate()
        {
            ...
            m_ValidSceneMask = EntityManager.GetEntityQueryMask(
                GetEntityQuery(new EntityQueryDesc
                {
                    All = new[]
                    {
                        ComponentType.ReadOnly<Entities.SceneReference>(), ComponentType.ReadOnly<RequestSceneLoaded>(), ComponentType.ReadOnly<AssetDependencyTrackerState>()
                    },
                    None = new[]
                    {
            ...
            m_AddScenes = GetEntityQuery(
                new EntityQueryDesc
                {
                    All = new[]
                    {
                        ComponentType.ReadOnly<Entities.SceneReference>(), ComponentType.ReadOnly<RequestSceneLoaded>()
                    },
                    None = new[]
                    {
                        ComponentType.ReadOnly<DisableSceneResolveAndLoad>(),
            ...
            m_RemoveScenes = GetEntityQuery(
                new EntityQueryDesc
                {
                    ...
                    None = new[]
                    {
                        ComponentType.ReadOnly<Entities.SceneReference>(),
    ...
    partial class ResolveSceneReferenceSystem : SystemBase
    {
        ...
        protected unsafe override void OnUpdate()
        {
            Entities.WithStructuralChanges().WithNone<DisableSceneResolveAndLoad, ResolvedSectionEntity, RequestSceneHeader>().
                ForEach((Entity sceneEntity, ref Entities.SceneReference scene, ref RequestSceneLoaded requestSceneLoaded) =>
                {
            ...
            Entities.WithStructuralChanges().WithNone<DisableSceneResolveAndLoad, ResolvedSectionEntity>().ForEach(
                (Entity sceneEntity, ref RequestSceneHeader requestHeader, ref Entities.SceneReference scene, ref ResolvedSceneHash resolvedSceneHash, ref RequestSceneLoaded requestSceneLoaded) =>
                {
                    if (!requestHeader.IsCompleted)
                    {
