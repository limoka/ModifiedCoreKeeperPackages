    ...
    internal class SpriteEditorMenu : EditorWindow
    {
        ...
        private void DoPivotGUI()
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                s_Setting.pivot = SpriteEditorUtility.GetPivotValue((UnityEngine.SpriteAlignment)alignment, s_Setting.pivot);
            }

            Vector2 pivot = s_Setting.pivot;
            EditorGUI.BeginChangeCheck();
            using (new EditorGUI.DisabledScope(alignment != (int)UnityEngine.SpriteAlignment.Custom))
            {
