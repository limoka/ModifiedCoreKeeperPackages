...
namespace UnityEditor.U2D.Sprites
{
    internal abstract partial class SpriteFrameModuleBase : SpriteEditorModuleBase
    {
        ...
        private void AddMainUI(VisualElement mainView)
        {
            ...
            m_PivotField.Init(UnityEngine.SpriteAlignment.Center);
            m_PivotField.label = L10n.Tr("Pivot");
            m_PivotField.RegisterValueChangedCallback((evt) =>
            {
                if (hasSelected)
                {
                    UnityEngine.SpriteAlignment alignment = (UnityEngine.SpriteAlignment)evt.newValue;
                    SetSpritePivotAndAlignment(selectedSpritePivot, alignment);
                    m_CustomPivotElement.SetEnabled(selectedSpriteAlignment == UnityEngine.SpriteAlignment.Custom);
        ...
        protected void PopulateSpriteFrameInspectorField()
        {
            ...
            m_CustomPivotElement.SetEnabled(hasSelected && selectedSpriteAlignment == UnityEngine.SpriteAlignment.Custom);
        }

        private static Vector2 ApplySpriteAlignmentToPivot(Vector2 pivot, Rect rect, UnityEngine.SpriteAlignment alignment)
        {
            if (alignment != UnityEngine.SpriteAlignment.Custom)
            {
        ...
        private static Vector2[] GetSnapPointsArray(Rect rect)
        {
            ...
            snapPoints[(int)UnityEngine.SpriteAlignment.TopLeft] = new Vector2(rect.xMin, rect.yMax);
            snapPoints[(int)UnityEngine.SpriteAlignment.TopCenter] = new Vector2(rect.center.x, rect.yMax);
            snapPoints[(int)UnityEngine.SpriteAlignment.TopRight] = new Vector2(rect.xMax, rect.yMax);
            snapPoints[(int)UnityEngine.SpriteAlignment.LeftCenter] = new Vector2(rect.xMin, rect.center.y);
            snapPoints[(int)UnityEngine.SpriteAlignment.Center] = new Vector2(rect.center.x, rect.center.y);
            snapPoints[(int)UnityEngine.SpriteAlignment.RightCenter] = new Vector2(rect.xMax, rect.center.y);
            snapPoints[(int)UnityEngine.SpriteAlignment.BottomLeft] = new Vector2(rect.xMin, rect.yMin);
            snapPoints[(int)UnityEngine.SpriteAlignment.BottomCenter] = new Vector2(rect.center.x, rect.yMin);
            snapPoints[(int)UnityEngine.SpriteAlignment.BottomRight] = new Vector2(rect.xMax, rect.yMin);
            return snapPoints;
        }
        ...
        protected void HandlePivotHandle()
        {
            ...
            UnityEngine.SpriteAlignment alignment = selectedSpriteAlignment;
